 ...
 * @requires Mesh.js
 */
neo3d.PlaneMesh = function(width, height, vSubdivs, hSubdivs, bGenTexCoords)
{
	...
	var indexType = neo3d.Mesh.INDEX_TYPE.NO_INDEX;
	if (nbVtx > 4)
		indexType = neo3d.Mesh.INDEX_TYPE.UNSIGNED_SHORT;

	neo3d.Mesh.call(this, neo3d.Mesh.PRIMITIVE_TYPE.TRIANGLE_STRIP, indexType);
...
neo3d.PlaneMesh.prototype = new neo3d.Mesh();

neo3d.PlaneMesh.prototype._initAttribConfig = function()
{
	var attribConfig = new neo3d.Mesh.AttribConfig();
	if (this._bGenTexCoords)
		attribConfig.stride = 20; //size of [x, y, z, u, v] floats

	this._attribConfigList[neo3d.Mesh.ATTRIB_TYPE.VERTEX] = attribConfig;

	if (this._bGenTexCoords)
	{
		attribConfig = new neo3d.Mesh.AttribConfig();
		attribConfig.nbComponents = 2;
		attribConfig.offset = 12; //size of [x, y, z] floats
		attribConfig.stride = 20; //size of [x, y, z, u, v] floats

		this._attribConfigList[neo3d.Mesh.ATTRIB_TYPE.TEX_COORDS] = attribConfig;
	}

	this._pointCount = 2 * this._subdivs[0] * (this._subdivs[1] + 2) - 2;
};

neo3d.PlaneMesh.prototype._initVBO = function()
{
	var vtx;

	if (this._indexType === neo3d.Mesh.INDEX_TYPE.UNSIGNED_SHORT)
	{
		var idx = [],
			x = this._dim[0],
			y = this._dim[1],
