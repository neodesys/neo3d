 ...
 * @requires neo3d.js
 */
neo3d.Mesh = function(primitiveType, indexType)
{
	if ((typeof(primitiveType) === "number") &&
		(primitiveType > 0) && (primitiveType < neo3d.Mesh.PRIMITIVE_TYPE.NB_ELEMS))
		this._primitiveType = primitiveType;
	else
		this._primitiveType = 0;

	if ((typeof(indexType) === "number") &&
		(indexType > 0) && (indexType < neo3d.Mesh.INDEX_TYPE.NB_ELEMS))
...
neo3d.Mesh.AttribConfig = function()
{
	this.vboIdx = 0;			//associated VBO index in _attribVBOList array
	this.componentType = neo3d.Mesh.COMPONENT_TYPE.FLOAT;
								//one of neo3d.Mesh.COMPONENT_TYPE values
	this.nbComponents = 3;		//from 1 to 4
	this.offset = 0;			//in bytes
	this.stride = 0;			//in bytes, maximum stride in WebGL is 255
	this.bNormalized = false;	//if true, values stored in an integer format
								//are mapped to [-1.f, 1.f] or [0.f, 1.f]
};

neo3d.Mesh.PRIMITIVE_TYPE = Object.freeze({
	NO_PRIMITIVE: 0,
	POINTS: 1,
	LINES: 2,
	LINE_STRIP: 3,
...
neo3d.Mesh.INDEX_TYPE = Object.freeze({
	NO_INDEX: 0,
	UNSIGNED_SHORT: 1,
	UNSIGNED_INT: 2,
	NB_ELEMS: 3
...
neo3d.Mesh.COMPONENT_TYPE = Object.freeze({
	NO_COMPONENT: 0,
	BYTE: 1,
	UNSIGNED_BYTE: 2,
	SHORT: 3,
...
neo3d.Mesh.ATTRIB_TYPE = Object.freeze({
	VERTEX: 0,
	TEX_COORDS: 1,
	VERTEX_COLOR: 2,
	NORMAL: 3,
...
neo3d.Mesh.prototype._initAttribConfig = function()
{
	//Override this method to populate this._attribConfigList array with
	//neo3d.Mesh.AttribConfig objects and set this._pointCount value.
...
neo3d.Mesh.prototype._initVBO = function()
{
	...
	//neo3d.Mesh.INDEX_TYPE.UNSIGNED_INT at object construction, its value may
	//have been switched to neo3d.Mesh.INDEX_TYPE.UNSIGNED_SHORT during mesh
...
neo3d.Mesh.prototype._createStaticVBO = function(data, bIndexVBO)
{
		...
		data && (data.buffer instanceof ArrayBuffer))
	{
		...
		if (vbo)
		{
			...
			if (error !== this._gl.NO_ERROR)
			{
				...
				{
					console.warn("cannot create static VBO (err: #" + error + ")");
...
neo3d.Mesh.prototype.init = function(gl, bContextLost)
{
	...
	//Indeed, this._indexType equals neo3d.Mesh.INDEX_TYPE.NO_INDEX or
	...
	if (!this._attribConfigList)
	{
		...
		if ((this._indexType === neo3d.Mesh.INDEX_TYPE.UNSIGNED_INT) &&
			!this._gl._bExtendedIndexType)
		{
			this._indexType = neo3d.Mesh.INDEX_TYPE.UNSIGNED_SHORT;
	...
	if ((this._indexType !== neo3d.Mesh.INDEX_TYPE.NO_INDEX) && !this._indexVBO)
	{
...
neo3d.Mesh.prototype.shut = function()
{
	...
	{
		this._indexType = neo3d.Mesh.INDEX_TYPE.UNSIGNED_INT;
		delete this._bHadExtendedIndexType;
	}
};

neo3d.Mesh.prototype.isReady = function(gl)
{
	//If this._attribVBOList is valid, then this._gl, this._attribConfigList
	//and this._pointCount are guaranteed to be valid.
	//Indeed, this._indexType equals neo3d.Mesh.INDEX_TYPE.NO_INDEX or
...
neo3d.Mesh.prototype.getPrimitiveType = function()
{
	return this._primitiveType;
};

neo3d.Mesh.prototype.getIndexType = function()
{
	return this._indexType;
};

neo3d.Mesh.prototype.getIndexVBO = function()
{
	return this._indexVBO;
};

neo3d.Mesh.prototype.getPointCount = function()
{
	return this._pointCount;
};

neo3d.Mesh.prototype.bindAttribBuffer = function(location, attribType, layer)
{
	//If this._attribVBOList is valid, then this._gl, this._attribConfigList
	//and this._pointCount are guaranteed to be valid.
		...
		(attribType < this._attribConfigList.length))
	{
		...
		if ((attribConfig instanceof neo3d.Mesh.AttribConfig) &&
			...
			(attribConfig.vboIdx < this._attribVBOList.length))
		{
			...
			if (vbo)
			{
				...
				{
				case neo3d.Mesh.COMPONENT_TYPE.BYTE:
					compType = this._gl.BYTE;
					break;

				case neo3d.Mesh.COMPONENT_TYPE.UNSIGNED_BYTE:
					compType = this._gl.UNSIGNED_BYTE;
					break;

				case neo3d.Mesh.COMPONENT_TYPE.SHORT:
					compType = this._gl.SHORT;
					break;

				case neo3d.Mesh.COMPONENT_TYPE.UNSIGNED_SHORT:
					compType = this._gl.UNSIGNED_SHORT;
					break;

				case neo3d.Mesh.COMPONENT_TYPE.FLOAT:
