...
{
    assert.equal(Quat.NB_COMPONENTS, 4);
    assert.bufferEqual(Quat.IDENTITY.buffer, [0, 0, 0, 1]);
});

QUnit.test("createBuffer", function(assert)
{
    var buffer = Quat.createBuffer(2);
    assert.equal(buffer.length, 2 * Quat.NB_COMPONENTS);
...
{
    var axis = new Vec3().setFromValues(0.0000001, 0.0000002, 0.0000003);
...
{
    var v0 = new Vec3().setFromValues(0.0000001, 0.0000002, 0.0000003),
        v1 = new Vec3().setFromValues(0.0000001, 0.0000002, 0.0000003);
...
{
    var q = new Quat().setFromArray(this.randomVal);
...
{
    var axis = new Vec3(),
        expectedAxis = new Vec3().setFromValues(0.2, 2.1, -1.4).normalizeInPlace();
    ...
    assert.equal(this.quat.getAxisAndAngle(axis), math.TWO_PI);
    assert.bufferEqual(axis.buffer, [1, 0, 0]);
});
...
QUnit.test("equals", function(assert)
{
    ...
    this.quat.buffer[1] += math.EPSILON;
    assert.bufferEqualish(this.quat.buffer, q.buffer);
...
{
    var q = new Quat().setFromArray(this.randomVal).normalizeInPlace();
...
{
    var q = new Quat().setFromValues(4.5, 1.2, 6.7, 0.6);
...
{
    var q0 = new Quat().setFromArray(this.randomVal),
        q1 = new Quat().negate(q0);
...
{
    var q0 = new Quat().setFromValues(1.2, -2.2, 3.4, -1.6),
        q1 = new Quat().setFromValues(-3.4, 1.7, -2.9, 0.8);
...
{
    var q = new Quat().setFromValues(-3.4, 1.7, -2.9, 0.8);
...
{
    var q = new Quat().setFromValues(0.0000001, 0.0000002, 0.0000003, 0.0000004);
...
{
    var q = new Quat().setFromValues(8.6, 4.5, 1.7, 0.9);
...
{
    var q = new Quat().setFromValues(1.5, 0.6, 1.7, 2.2);
...
{
    var q = new Quat().setFromValues(0.0000001, 0.0000002, 0.0000003, 0.0);
    ...
    q.buffer[3] = -2.3;
    assert.bufferEqualish(this.quat.log(q).buffer, [math.PI, 0, 0, 0.8329091]);
...
QUnit.test("logInPlace", function(assert)
{
    ...
    this.quat.buffer[3] = -2.3;
    assert.bufferEqualish(this.quat.logInPlace().buffer, [math.PI, 0, 0, 0.8329091]);
...
{
    var q0 = new Quat().setFromValues(0.2, 4.7, 2.2, 1.4),
        q1 = new Quat().setFromValues(1.7, -2.8, 2.7, -1.6);
...
{
    var q0 = new Quat().setFromValues(0.037184, 0.8738233, 0.4090237, 0.2602878),
        q1 = new Quat().setFromValues(0.3747367, -0.6172134, 0.5951701, -0.3526934);
...
{
    var s0 = new Quat().setFromValues(0.2262019, 0.7313482, 0.6159059, 0.1860711),
        q0 = new Quat().setFromValues(0.2148345, 0.6445034, 0.5370861, 0.5),
        q1 = new Quat().setFromValues(0.0418189, 0.1254567, 0.2090945, 0.9689124),
        s1 = new Quat().setFromValues(0.0062012, -0.0492608, 0.2737469, 0.9605194);
...
{
    var q0 = new Quat().setFromValues(0.2041241, 0.4082483, 0.2041241, 0.8660254),
        q1 = new Quat().setFromValues(0.2148345, 0.6445034, 0.5370861, 0.5),
        q2 = new Quat().setFromValues(0.0418189, 0.1254567, 0.2090945, 0.9689124),
        q3 = new Quat().setFromValues(-0.0311256, 0.1556279, -0.4980091, 0.8525245);
