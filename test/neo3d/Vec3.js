...
{
    assert.equal(Vec3.NB_COMPONENTS, 3);
    assert.bufferEqual(Vec3.I.buffer, [1, 0, 0]);
    assert.bufferEqual(Vec3.J.buffer, [0, 1, 0]);
    assert.bufferEqual(Vec3.K.buffer, [0, 0, 1]);
});

QUnit.test("createBuffer", function(assert)
{
    var buffer = Vec3.createBuffer(2);
    assert.equal(buffer.length, 2 * Vec3.NB_COMPONENTS);
...
{
    var v3 = new Vec3().setFromArray(this.randomVal);
    assert.bufferEqual(this.vec.copy(v3).buffer, this.randomVal);
});

QUnit.test("isNull", function(assert)
{
    this.vec.setFromValues(0.0000001, 0.0000002, 0.0000003);
...
QUnit.test("equals", function(assert)
{
    ...
    assert.bufferEqualish(this.vec.buffer, v3.buffer);
    assert.notOk(this.vec.equals(v3));

    this.vec.buffer[1] += 0.1;
    assert.notBufferEqualish(this.vec.buffer, v3.buffer);
    assert.notOk(this.vec.equals(v3));
});

QUnit.test("add", function(assert)
{
    var v0 = new Vec3().setFromValues(1.2, 1.6, 3.2),
        v1 = new Vec3().setFromValues(0.6, 5.3, 1.6);
...
{
    var v3 = new Vec3().setFromValues(0.6, 5.3, 1.6);
...
{
    var v0 = new Vec3().setFromValues(2.6, 3.2, 0.6),
        v1 = new Vec3().setFromValues(1.4, 2.1, 1.4);
...
{
    var v3 = new Vec3().setFromValues(1.4, 2.1, 1.4);
...
{
    var v3 = new Vec3().setFromValues(4.5, 1.2, 6.7);
...
{
    var v0 = new Vec3().setFromArray(this.randomVal),
        v1 = new Vec3().negate(v0);
...
{
    var v3 = new Vec3().setFromValues(1.8, 2.4, 0.7);
...
{
    var v3 = new Vec3().setFromValues(8.6, 4.5, 1.7);
    assert.bufferEqualish(this.vec.normalize(v3).buffer, [0.8727479, 0.4566704, 0.1725199]);
    assert.equalish(this.vec.norm(), 1.0);

    v3.setFromValues(0.0000001, 0.0000002, 0.0000003);
    assert.bufferEqual(this.vec.normalize(v3).buffer, [0, 0, 0]);
});
...
QUnit.test("normalizeInPlace", function(assert)
{
    ...
    this.vec.setFromValues(0.0000001, 0.0000002, 0.0000003);
    assert.bufferEqual(this.vec.normalizeInPlace().buffer, [0, 0, 0]);
});

QUnit.test("dotProduct", function(assert)
{
    var v3 = new Vec3().setFromValues(1.5, 0.6, 1.7);
...
{
    var v0 = new Vec3().setFromValues(1.4, 2.2, -1.3),
        v1 = new Vec3().setFromValues(6.3, -4.7, 2.1);
...
{
    var v3 = new Vec3().setFromValues(6.3, -4.7, 2.1);
...
{
    var v3 = new Vec3().setFromValues(6.3, -4.7, 2.1);
    assert.notEqualish(v3.norm(), 1.0);
    this.vec.setFromValues(0.0000001, 0.0000002, 0.0000003);

    assert.equal(this.vec.shortestAngle(v3), 0.0);
    assert.equal(v3.shortestAngle(this.vec), 0.0);

    this.vec.setFromValues(1.4, 2.2, -1.3);
    assert.notEqualish(this.vec.norm(), 1.0);

    assert.equalish(this.vec.shortestAngle(v3), 1.7510538);
    assert.equalish(v3.shortestAngle(this.vec), 1.7510538);

    this.vec.negateInPlace();
    assert.equalish(this.vec.shortestAngle(v3), math.PI - 1.7510538);

    this.vec.setFromValues(0.6, 1.6, 1.7809524); //perpendicular to v3
    assert.equal(this.vec.shortestAngle(v3), math.HALF_PI);

    this.vec.copy(v3).scaleInPlace(1.3); //colinear to v3, same direction
    assert.equal(this.vec.shortestAngle(v3), 0.0);

    this.vec.negateInPlace(); //colinear to v3, opposite direction
    assert.equal(this.vec.shortestAngle(v3), math.PI);
});

QUnit.test("isPerpendicular", function(assert)
{
    assert.ok(Vec3.I.isPerpendicular(Vec3.J));
    assert.ok(Vec3.J.isPerpendicular(Vec3.K));
    assert.ok(Vec3.K.isPerpendicular(Vec3.I));

    var v3 = new Vec3().setFromValues(1.5, 2.4, -1.1);
    assert.notOk(this.vec.isPerpendicular(v3));
    assert.notOk(v3.isPerpendicular(this.vec));

    this.vec.setFromValues(2.3, 5.1, 14.2636364);
    assert.ok(this.vec.isPerpendicular(v3));
    assert.ok(v3.isPerpendicular(this.vec));
...
{
    var p0 = new Vec3().setFromValues(0.2, 4.7, 2.2),
        p1 = new Vec3().setFromValues(1.7, -2.8, 2.7);
...
{
    var p0 = new Vec3().setFromValues(1.3, -0.4, 2.5),
        p1 = new Vec3().setFromValues(2.4, 1.2, 4.7),
        p2 = new Vec3().setFromValues(3.5, 0.4, 6.9);
...
{
    var t0 = new Vec3().setFromValues(1.5, -0.5, -2.85),
        p0 = new Vec3().setFromValues(2.1, 3.2, 4.1),
        p1 = new Vec3().setFromValues(4.2, 1.4, -2.6),
        t1 = new Vec3().setFromValues(1.55, -0.45, -3.65);
...
{
    var ctrl0 = new Vec3().setFromValues(2.6, 3.0333333, 3.15),
        p0 = new Vec3().setFromValues(2.1, 3.2, 4.1),
        p1 = new Vec3().setFromValues(4.2, 1.4, -2.6),
        ctrl1 = new Vec3().setFromValues(3.6833333, 1.55, -1.3833333);
...
{
    var p0 = new Vec3().setFromValues(1.2, 2.4, 3.1),
        p1 = new Vec3().setFromValues(2.1, 3.2, 4.1),
        p2 = new Vec3().setFromValues(4.2, 1.4, -2.6),
        p3 = new Vec3().setFromValues(5.2, 2.3, -3.2);
