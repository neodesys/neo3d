...
{
	assert.equal(neo3d.Mat2.NB_COMPONENTS, 4);
	assert.bufferEqual(neo3d.Mat2.IDENTITY.buffer, [1, 0, 0, 1]);
});

QUnit.test("createBuffer", function(assert)
{
	var buffer = neo3d.Mat2.createBuffer(2);
	assert.equal(buffer.length, 2 * neo3d.Mat2.NB_COMPONENTS);
...
QUnit.test("setColumnVec2", function(assert)
{
	var v0 = new neo3d.Vec2().setFromValues(this.randomVal[0], this.randomVal[1]),
		v1 = new neo3d.Vec2().setFromValues(this.randomVal[2], this.randomVal[3]);
...
{
	var s = new neo3d.Vec2().setFromValues(1.2, 2.3);
...
{
	var m2 = new neo3d.Mat2().setFromArray([
...
{
	var m0 = new neo3d.Mat2().setFromArray([
			1.2, 0.5, //Col0
			-0.5, 2.7 //Col1
		]),
		m1 = new neo3d.Mat2().setFromArray([
			-4.1, 4.4, //Col0
			2.7, 0.2   //Col1
		]);
...
{
	assert.bufferEqual(this.mat.transpose(neo3d.Mat2.IDENTITY).buffer, [1, 0, 0, 1]);
